#!/usr/bin/env bash
# =====================================================================
#  n8n + PostgreSQL backup script for staging
# =====================================================================
...
POSTGRES_CONTAINER=${POSTGRES_CONTAINER:-postgresdb_staging}
DB_NAME=${POSTGRES_DB:-n8n_database}
N8N_VOLUME_NAME=${N8N_VOLUME_NAME:-n8n_data_staging}
...
BACKUP_DIR="$N8N_BACKUP_DIR_HOST/staging"
...
echo "=================================================================="
echo "BẮT ĐẦU backup STAGING lúc:  $(date '+%F %T')"
...
echo "------------------------------------------------------------------"
if [[ "$FAILED" -eq 0 ]]; then
  echo "✅  Backup STAGING hoàn tất thành công."
else
  echo "❌  Backup STAGING hoàn tất nhưng CÓ LỖI (xem log)."
fi
