#!/bin/bash
# deploy-n8n.dev.sh - Script triển khai n8n cho môi trường development

# Hiển thị banner
echo "=============================================="
echo "     TRIỂN KHAI N8N SERVER - DEVELOPMENT     "
echo "=============================================="
...
# Tạo thư mục n8n_data_dev trước nếu chưa tồn tại
if [ ! -d "./n8n_data_dev" ]; then
  mkdir -p ./n8n_data_dev
  log "✅ Đã tạo thư mục n8n_data_dev"
fi

# Sao lưu file config hiện tại nếu có
if [ -f "./n8n_data_dev/config" ]; then
  log "📦 Sao lưu file config hiện tại..."
  cp "./n8n_data_dev/config" "./n8n_data_dev/config.bak"
...
CONFIG_ISSUE=false
if [ -f "./n8n_data_dev/config" ]; then
  if grep -q "# <<<=== THAY BẰNG KHÓA MẠNH CỦA BẠN" "./n8n_data_dev/config"; then
...
docker-compose -f docker-compose.yml -f docker-compose.dev.yml pull || handle_error "Không thể pull images"
...
docker-compose -f docker-compose.yml -f docker-compose.dev.yml build || handle_error "Không thể build images"
...
docker-compose -f docker-compose.yml -f docker-compose.dev.yml up -d || handle_error "Không thể khởi động containers"
...
if ! docker ps | grep -q "n8n_main_dev"; then
  log "⚠️ Container n8n_main_dev không đang chạy, kiểm tra logs..."
  docker logs n8n_main_dev --tail 50
  handle_error "Container n8n_main_dev không khởi động được"
fi
log "✅ Container n8n_main_dev đang chạy"

# 9. Kiểm tra file config
log "🔍 Kiểm tra file config của n8n..."
CONFIG_CONTENT=$(docker exec n8n_main_dev cat /home/node/.n8n/config 2>/dev/null)
if [ $? -ne 0 ]; then
...
if docker exec n8n_main_dev bash -c "pip3 list | grep -q msoffcrypto-tool"; then
  MSOFFCRYPTO_VERSION=$(docker exec n8n_main_dev bash -c "pip3 list | grep msoffcrypto-tool" | awk '{print $2}')
...
echo "   http://localhost:5680"
if [ ! -z "$WEBHOOK_URL" ]; then
  echo "   hoặc: $WEBHOOK_URL"
fi

log "✅ TRIỂN KHAI N8N DEVELOPMENT HOÀN TẤT THÀNH CÔNG"
exit 0 