...
public class UserRepository : BaseRepository<User, Guid>, IUserRepository
{
    ...
    {
        return await _dbSet.AnyAsync(u => u.Email == email && u.Id != excludeUserId, cancellationToken);
    }
    ...
    {
        return await _dbSet.AnyAsync(u => u.Username == username && u.Id != excludeUserId, cancellationToken);
    }
    ...
    {
        var query = _dbSet.AsQueryable();        if (!string.IsNullOrEmpty(search))
        {
            query = query.Where(u => u.FullName.Contains(search) || 
                                   u.Email.Contains(search) || 
                                   u.Username.Contains(search));
